...
import { useFetchSurveyDetail } from 'app/pages/SurveyDetail/hooks/useFetchSurveyDetail';
...
export const SurveyDetail = () => {
  ...
  const { isLoading, data, refetch } = useFetchSurveyDetail({ id });
  ...
  const initialValues = useMemo((): TFormBody => {
    return {
      ...
      applyTime: data?.applyTime ? dayjs(data?.applyTime).toDate() : undefined,
      endTime: data?.endTime ? dayjs(data?.endTime).toDate() : undefined,
  ...
  return (
    <Box pos="relative" className="form-wrapper">
      ...
      {isLoading ? null : data ? (
        ...
        <Navigate to={Routes.SURVEY} replace />
      )}
    </Box>
  );
};
